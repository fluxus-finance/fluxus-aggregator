...
use near_sdk::{ext_contract, log, near_bindgen, Balance, PromiseOrValue};
...
fn try_identify_pool_id(token_id: &String) -> Result<u64, &'static str> {
    ...
    log!("{}", token_id.starts_with(":"));
    log!("Aqui ta dentro da try (onde é passado o endereço):{}", token_id);
...
fn parse_token_id(token_id: String) -> TokenOrPool {
    log!("Aqui ta dentro do parse : {}", token_id);
